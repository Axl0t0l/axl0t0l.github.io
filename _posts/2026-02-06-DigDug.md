---
title: DigDug | TryHackMe
date: 2026-02-06 12-00-00 +0200
categories: [TryHackMe, Challenges]
tags: [misc, machines]     
---

![](/assets/img/logos/THM-logo.png){: w="400" h="400" }

## Overview
- **Platform:** [Tryhackme](https://tryhackme.com/)
- **Challenge:** [DigDug](https://tryhackme.com/room/digdug)
- **Categories:** Misc, Machines
- **Rating:** 8/10
- **Difficulty:** Very Easy
- **Sovling Time:** ~3 minutes

## Challenge
> "*Oooh, turns out, this MACHINE_IP machine is also a DNS server! If we could dig into it, I am sure we could find some interesting records! But... it seems weird, this only responds to a special type of request for a givemetheflag.com domain?*"

## Solution Approach
### Recon
Always start with nmap scan, since we're looking for DNS server we should use ```-sU``` flag so its

```bash nmap -sC -sU -p 50-100 MACHINE_IP```

![](/assets/img/posts/2026-02-06-DigDug/nmap.png){: .normal }

> [!INFO]
>
> I only used ```-p``` flag in nmap to reduce the scanning time, otherwise you can scan normally.



### Walkthrough
Hmm looks we have DNS server at port 53 lets use nslookup to check its TXT records!

```bash nslookup -type=txt givemetheflag.com MACHINE_IP```

and Tatadaa!

![](assets/img/posts/2026-02-06-DigDug/nslookup.png){: .normal }

***Hope you enjoined it (;***

## Flag
```flag{0767ccd06e79853318f25aeb08ff83e2}```
